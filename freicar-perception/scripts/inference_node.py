...
def postprocess(image, da_pred, ll_pred):
    img_rs=image.copy()
...
class Inferencer(object):
    def __init__(self):
        ...
        self.publisher_lookahead_point = rospy.Publisher("/freicar_4/lookahead_point", Pose, queue_size=10)

        #self.publisher_da_out = rospy.Publisher("/freicar_3/inference_da", Image, queue_size=1)
        #self.publisher_ll_out = rospy.Publisher("/freicar_3/inference_ll", Image, queue_size=1)

        self.image_height = None
        self.image_width = None

        print("Running TensorRT inference for TwinLiteNet")
        cuda.init()
        self.cuda_ctx = cuda.Device(0).make_context()

        self.engine_file = "/home/freicar/freicar_ws/src/freicar-perception/scripts/flowlitenet.engine" # "flowlitenet_fp16.engine"
        self.engine = load_engine(self.engine_file)
        ...
        for i in range(self.engine.num_io_tensors):
                ...
                if self.engine.get_tensor_mode(name) == trt.TensorIOMode.INPUT:
                    self.image_height = shape[2]
                    self.image_width = shape[3]
                    print (" is input\n")
                else:  # output
                    print (" is output\n")
        ...
        for i in range(self.engine.num_io_tensors):
            ...
            if self.engine.get_tensor_mode(tensor_name) == trt.TensorIOMode.INPUT:
                #self.input_buffer = cuda.pagelocked_empty(size, dtype) #np.ascontiguousarray(input_image)
                #print("input buffer bytes: {}, my calculation: {}".format(self.input_buffer.nbytes, size*4))
                self.input_memory = cuda.mem_alloc(size*4) #(input_buffer.nbytes)
                self.bindings.append(int(self.input_memory))
            else:
                output_buffer = cuda.pagelocked_empty(size, dtype)
                self.output_buffers.append(output_buffer)
                output_memory = cuda.mem_alloc(output_buffer.nbytes)
                self.output_mems.append(output_memory)
                self.bindings.append(int(output_memory))


        #self.subscriber = rospy.Subscriber("/freicar_3/d435/color/image_raw", Image, self.image_callback, queue_size=1, buff_size = 16777216)
        #self.subscriber = rospy.Subscriber("/freicar_3/d435/color/image_raw/compressed", CompressedImage, self.image_callback, queue_size=1, buff_size = 16777216)
    ...
    def infer(self, input_img, depth, header):
        ...
        self.cuda_ctx.push()
        #np.copyto(self.input_buffer, input_image.ravel())
        cuda.memcpy_htod_async(self.input_memory, np.ascontiguousarray(input_image), self.stream) #self.input_buffer
        ...
        for out_idx in range(len(self.output_buffers)):
            cuda.memcpy_dtoh_async(self.output_buffers[out_idx], self.output_mems[out_idx], self.stream)
        ...
        da = np.reshape(self.output_buffers[0], (2, self.image_height, self.image_width))
        ll = np.reshape(self.output_buffers[1], (2, self.image_height, self.image_width))

        da_pred = (np.argmax(da, 0) * 255).astype(np.uint8)
        ll_pred = (np.argmax(ll, 0) * 255).astype(np.uint8)

        #da_pred_img = self.bridge.cv2_to_imgmsg(da_pred, encoding="mono8")
        #da_pred_img.header = header
        #self.publisher_da_out.publish(da_pred_img)
        #ll_pred_img = self.bridge.cv2_to_imgmsg(ll_pred, encoding="mono8")
        #ll_pred_img.header = header
        #self.publisher_ll_out.publish(ll_pred_img)
    ...
    def image_callback(self, msg, msg_depth):
        ...
        img_out, cc_im, cc_im_rl, path = self.infer(cv_image, depth_metric, msg.header)
        ...
        if path is not None:
            ...
            lookahead_point.position.x = path[0][index]
            lookahead_point.position.y = path[1][index]

            self.publisher_lookahead_point.publish(lookahead_point)
