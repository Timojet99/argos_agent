...
freicar::agent::FreiCarCarlaProxy::Settings GetAgentParameters() {
    ...
    if (!ros::param::get("/comp_mode", settings.comp_mode)) {
        ROS_ERROR("ERROR: could not find global parameter: comp_mode");
        std::exit(EXIT_FAILURE);
    }

    if (!ros::param::get("/sim_sync_mode", settings.sim_sync_mode)) {
        ROS_ERROR(
            "ERROR: could not find global parameter: sim_sync_mode, Make sure that the freicar_setting node is "
            "running");
        std::exit(EXIT_FAILURE);
    }
    ...
    if (!ros::param::get("~spawn/heading", settings.spawn_heading)) {
        ROS_ERROR("ERROR: could not find parameter: spawn/heading");
        std::exit(EXIT_FAILURE);
    }
    ...
    if (!ros::param::get("~use_yaml_spawn", settings.use_yaml_spawn)) {
        ROS_ERROR("ERROR: could not find parameter: use_yaml_spawn");
        std::exit(EXIT_FAILURE);
    }
    ...
    if (!ros::param::get("/map_path", settings.map_path)) {
        if (settings.use_yaml_spawn) {
            ROS_ERROR("could not find global parameter: map_path! Provide the path or turn use_yaml_spawn off.");
            std::exit(EXIT_FAILURE);
        }
    }
    ...
    if (type_string == "sim") {
        ...
        if (!ros::param::get("/sim_sps", steps_per_second) || !ros::param::has("/sim_sync_mode") ||
            !ros::param::has("/sim_norender")) {
            ROS_ERROR("ERROR: could not find simulation parameters");
            std::exit(EXIT_FAILURE);
        }
        // in sim_only, the delay is determined by the sim config
        settings.thread_sleep_ms =
            (steps_per_second != 0) ? ((1.0 / steps_per_second) * 1000) : settings.thread_sleep_ms;
...
int main(int argc, char** argv) {
    ...
    switch (settings.type_code) {
        ...
        case freicar::agent::type::REAL | freicar::agent::type::SIMULATED:
            ...
            break;
        default:
            std::cout << "invalid agent type code" << settings.type_code << std::endl;
