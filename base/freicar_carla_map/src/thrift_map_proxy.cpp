...
void ThriftMapProxy::StopMapServer() { server_->StopThreadedServer(); }

/* start the local thrift server */
void ThriftMapProxy::StartMapServer() { server_->StartThreadedServer(); }

/* a simple helper function to print the Map stats */
void ThriftMapProxy::PrintMapStats() {
    auto stats = Map::GetInstance().GetMapStats();
    std::cout << "thrift proxy: Lanes: " << std::get<0>(stats) << " LaneGroups: " << std::get<1>(stats)
...
map_thrift::MapMessage ThriftMapProxy::ReturnMapAsMessage() { return Map::GetInstance().AsThriftMessagae(); }
...
void ThriftMapProxy::ThriftServer::InitializeThriftServer() {
    ...
    apache::thrift::stdcxx::shared_ptr<MapCommHandler> handler(new ThriftMapProxy::MapCommHandler());
    apache::thrift::stdcxx::shared_ptr<TProcessor> processor(new map_thrift::MapCommProcessor(handler));
    ...
    thrift_server_object_ = new TSimpleServer(processor, serverTransport, transportFactory, protocolFactory);
...
void ThriftMapProxy::ThriftServer::ThriftServerThreadFunction() {
    if (thrift_server_object_) {
        std::cout << "thrift proxy: server thread #" << std::this_thread::get_id() << " running on port "
                  << thrift_server_port_ << std::endl;
        try {
...
bool ThriftMapProxy::ThriftClient::connected() { return connected_to_thrift_server_; }
...
void ThriftMapProxy::MapCommHandler::ping() { std::cout << "thrift proxy: received PING from remote" << std::endl; }
...
bool ThriftMapProxy::MapCommHandler::sendMapMessage(const map_thrift::MapMessage& map_complete) {
    std::cout << "thrift proxy: receiving " << ThriftMapProxy::GetMessageOpName(map_complete.op)
              << " map message from remote" << std::endl;
