#include "freicar_carla_map/logic/right_of_way.h"
...
static std::pair<int, float> GetCorrectJunctionID(const JunctionAgent& observed_agent, map::Map& map_instance) {
    ...
    // on a junction
    if (j_id != -1) return std::make_pair(j_id, 0.0f);
    ...
    if (!next_lane) return std::make_pair(-1, 0.0f);
...
bool JunctionAgent::IsOnRightHandSideOf(const JunctionAgent& observer_agent) const {
    ...
    observer_tf << std::cos(yaw), -std::sin(yaw), observer_agent.current_pose.transform.translation.x, std::sin(yaw),
        std::cos(yaw), observer_agent.current_pose.transform.translation.y, 0, 0, 1;
    ...
    if (new_pos.y() == 0) std::cout << "[ERROR] 0.000001 chance of hitting this edge case" << std::endl;
    if (new_pos.y() > 0) return false;
    return true;
}
...
bool JunctionAgent::HasRightOfWay(const JunctionAgent& observed_agent, bool ignore_intent) const {
    ...
    for (auto agent_sign : agent_lane_signs) {
        if (agent_sign->GetSignType() == "Stop") junction_signs |= JunctionSign::AGENT_STOP;
        if (agent_sign->GetSignType() == "RightOfWay") junction_signs |= JunctionSign::AGENT_ROW;
    }
    for (auto observed_sign : observed_lane_signs) {
        if (observed_sign->GetSignType() == "Stop") junction_signs |= JunctionSign::OBSERVED_STOP;
        if (observed_sign->GetSignType() == "RightOfWay") junction_signs |= JunctionSign::OBSERVED_ROW;
    }
        ...
        case AGENT_ROW | OBSERVED_ROW:
            if (this->IsOnRightHandSideOf(observed_agent) &&  // current on RHS of observed
                ...
                if (!ignore_intent && this->intent == Intent::GOING_LEFT)
                    return false;
                else if (this->intent == Intent::GOING_STRAIGHT || this->intent == Intent::GOING_RIGHT || ignore_intent)
                    return true;
                else
                    std::cout << "[WARNING] unhandled case: " << name << " and " << observed_agent.name
                              << " on LHS w/ unknown intent" << std::endl;
            } else
                std::cout << "[WARNING] unhandled case: " << name << " and " << observed_agent.name << " on RHS"
                          << std::endl;
            break;
        // case AGENT_STOP:
        // case OBSERVED_ROW:
...
std::tuple<bool, bool, std::string> GetRightOfWay(const JunctionAgent& agent,
                                                  const std::vector<JunctionAgent>& observed_agents,
                                                  bool consider_velocities, bool ignore_intent) {
    ...
    if (current_junction == -1) {
        if (map_instance.GetCurrentJunctionID(agent.lane_uuid) != -1) return std::make_tuple(true, true, agent.name);
    ...
    for (auto& observed_agent : observed_agents) {
        ...
        if (consider_velocities) {
            ...
            auto velo =
                std::sqrt(observed_agent.vx() * observed_agent.vx() + observed_agent.vy() * observed_agent.vy() +
