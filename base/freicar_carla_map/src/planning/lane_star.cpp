...
LaneStar::LaneStar(unsigned int max_steps) {
    ...
    // containers
    lane_cost_hash_ =
        new std::unordered_map<const mapobjects::Lane*, LaneCost, decltype(lane_hash_lambda_)>(10, lane_hash_lambda_);
    previous_hash_ =
        new std::unordered_map<const mapobjects::Lane*, const mapobjects::Lane*, decltype(lane_hash_lambda_)>(
            10, lane_hash_lambda_);
    queue_ = new std::priority_queue<const mapobjects::Lane*, std::vector<const mapobjects::Lane*>,
                                     decltype(compare_lambda_)>(compare_lambda_);
...
static float GetL1Distance(const mapobjects::Lane* l1, const mapobjects::Lane* l2) {
    ...
    return std::abs(ct1.x() - ct2.x()) + std::abs(ct1.y() - ct2.y()) + std::abs(ct1.z() - ct2.z());
}
...
static float GetApproximateCost(const mapobjects::Lane* lane, const mapobjects::Lane* neighbor) {
    ...
    switch (type) {
        ...
        case mapobjects::Lane::ADJACENT_RIGHT:
            return (lane->GetWidth() + neighbor->GetWidth()) / 2;
        default:
            std::cout << "ERROR: unhandled type between " << lane->GetUuid().GetUuidValue() << " and "
                      << neighbor->GetUuid().GetUuidValue() << "\n"
...
void LaneStar::ExpandLane(const mapobjects::Lane* lane) {
    ...
    for (auto neighbor : neighbors) {
        ...
        if (itr != lane_cost_hash_->end()) {
            ...
            if (updated_cost.f() >= itr->second.f()) continue;
            ...
            std::make_heap(const_cast<const mapobjects::Lane**>(&queue_->top()),
                           const_cast<const mapobjects::Lane**>(&queue_->top()) + queue_->size(), compare_lambda_);
        } else {
...
std::pair<std::vector<const mapobjects::Lane*>, LaneCost> LaneStar::GetLanes() {
    ...
    while (!queue_->empty()) {
        ...
        ExpandLane(current);
        if (++steps_ > maximum_steps_) break;
    }
...
freicar::planning::Plan LaneStar::GetPlan(const mapobjects::Point3D& start, float start_heading,
    ...
    auto start_lp =
        map_instace.FindClosestLanePointsWithHeading(start.x(), start.y(), start.z(), 3, start_heading)[0].first;
        ...
        if (lane_index != current_points.size()) {
            ...
            if (current_distance + r_dist < p2p_distance) {
                ...
                if (final_node) {
                    ...
                    lane_offset += r_dist;
                    plan_offset += r_dist;
                    plan.emplace_back(goal, Connection::STRAIGHT, current_lane->GetUuid().GetUuidValue(), lane_offset,
                                      plan_offset);
                ...
                plan.emplace_back(p_current, lane_connection, current_lane->GetUuid().GetUuidValue(), lane_offset,
                                  plan_offset);
