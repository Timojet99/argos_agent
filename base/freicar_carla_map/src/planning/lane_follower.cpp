...
Plan GetPlan(const std::string& lane_uuid, float req_loffset, enums::PlannerCommand command, float distance,
    ...
    if (std::abs(req_loffset - current_lane->GetLength()) <= 1e-3) {
        ...
        std::cout << "CRITICAL ERROR: requested lane offset " << req_loffset << " is larger than the lane's length "
                  << current_lane->GetLength() << std::endl;
        plan.success = false;
        return plan;
        // finding the point with that lane offset
    } else {
        for (size_t i = 0; i < current_points.size(); ++i) {
            if (i > 0) lane_offset += current_points[i - 1].ComputeDistance(current_points[i]);
    ...
    plan.emplace_back(p_current, Connection::STRAIGHT, current_lane->GetUuid().GetUuidValue(), lane_offset,
                      plan_offset);
...
*/
Plan GetPlan(const mapobjects::Point3D& current_position, enums::PlannerCommand command, float distance,
             unsigned int step_count) {
    using Connection = freicar::mapobjects::Lane::Connection;
    if (command > enums::RANDOM) {
        std::cout << "[ERROR] invalid planner command used with lane follower planner" << std::endl;
    ...
    auto p_closest =
        map.FindClosestLanePoints(current_position.x(), current_position.y(), current_position.z(), 1)[0].first;
