...
mapobjects::Pivot ToMapObjectsPivot(const map_thrift::Pivot& thrift_pivot) {
    return mapobjects::Pivot(
        mapobjects::Uuid(thrift_pivot.id.uuid),
        mapobjects::Pose(mapobjects::Point3D(thrift_pivot.pose.position.x, thrift_pivot.pose.position.y,
...
mapobjects::LaneMarkingContainer ToMapObjectLaneMarkingContainer(const map_thrift::LaneMarkingContainer& lmc) {
    mapobjects::LaneMarkingContainer mo_lmc =
        mapobjects::LaneMarkingContainer(ToMapObjectsUuids(lmc.left), ToMapObjectsUuids(lmc.right));
    return mo_lmc;
}
...
mapobjects::LaneMarking ToMapObjectsLanemarking(const map_thrift::LaneMarking& lm) {
    return mapobjects::LaneMarking(
        mapobjects::Uuid(lm.id.uuid),
        ...
        convert::ToMapObjectLaneMarkingType(lm.type), convert::ToMapObjectPoints(lm.points), lm.visibility);
}

/* converts a map_thrift::Lane to maobjects::Lane */
mapobjects::Lane ToMapObjectsLane(const map_thrift::Lane& lane) {
    return mapobjects::Lane(
        mapobjects::Uuid(lane.id.uuid), convert::ToMapObjectLaneMarkingContainer(lane.lane_markings),
        convert::ToMapObjectLaneDirection(lane.dir), convert::ToMapObjectsUuids(lane.incoming_connections),
        convert::ToMapObjectsUuids(lane.outgoing_connections), convert::ToMapObjectLaneType(lane.type),
        convert::ToMapObjectPoints(lane.points), lane.width,
        mapobjects::Point3D(lane.handle_point.x, lane.handle_point.y, lane.handle_point.z), lane.visibility,
        lane.height, lane.junction_id);
}
...
mapobjects::LaneGroup ToMapObjectsLaneGroup(const map_thrift::LaneGroup& lg) {
    return mapobjects::LaneGroup(mapobjects::Uuid(lg.id.uuid), convert::ToMapObjectsUuids(lg.lanes_left),
...
mapobjects::Stopline ToMapObjectsStopLine(const map_thrift::Stopline& stop_line) {
    return mapobjects::Stopline(mapobjects::Uuid(stop_line.lane_object.id.uuid),
                                ...
                                convert::ToMapObjectsUuids(stop_line.lane_object.l_ids), stop_line.lane_object.offset);
}
...
mapobjects::Crosswalk ToMapObjectsCrossWalk(const map_thrift::Crosswalk& crosswalk) {
    return mapobjects::Crosswalk(mapobjects::Uuid(crosswalk.lane_object.id.uuid),
                                 ...
                                 convert::ToMapObjectsUuids(crosswalk.lane_object.l_ids), crosswalk.lane_object.offset);
}
...
mapobjects::Parking ToMapObjectsParking(const map_thrift::Parking& parking_lot) {
    ...
    return mapobjects::Parking(mapobjects::Uuid(parking_lot.lane_object.id.uuid),
                               ...
                               parking_lot.lane_object.offset, parking_lot.number_of_lots, parking_lot.parking_height,
                               parking_lot.parking_width, parking_lot.line_width, outline_list);
}
...
mapobjects::Roadsign ToMapObjectsRoadsign(const map_thrift::Roadsign& roadsign) {
    return mapobjects::Roadsign(
        mapobjects::Uuid(roadsign.lane_object.id.uuid), mapobjects::Uuid(roadsign.lane_object.lg_id.uuid),
        convert::ToMapObjectsUuids(roadsign.lane_object.l_ids), 0, roadsign.sign_type,
        mapobjects::Point3D(roadsign.position.x, roadsign.position.y, roadsign.position.z), roadsign.rotation);
}
...
map_thrift::Pivot ToThriftPivot(const mapobjects::Pivot& map_pivot) {
    ...
    t_pivot.pose.position = convert::ToThriftPoint(
        mapobjects::Point3D(map_pivot.GetPose().x(), map_pivot.GetPose().y(), map_pivot.GetPose().z()));
