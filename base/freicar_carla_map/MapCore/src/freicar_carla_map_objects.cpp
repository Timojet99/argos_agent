#include "map_core/freicar_carla_map_objects.h"
...
Uuid MapObject::GetUuid() const { return id_; }
// getter for Uuid
std::string Uuid::GetUuidValue() const { return uuid_; }

// getters for Point3D
std::tuple<float, float, float> Point3D::GetCoords() const { return std::make_tuple(x_, y_, z_); }
float Point3D::x() const { return x_; }
float Point3D::y() const { return y_; }
float Point3D::z() const { return z_; }
/* minus operator overload */
Point3D Point3D::operator-(const Point3D& rhs) const { return Point3D(x_ - rhs.x_, y_ - rhs.y_, z_ - rhs.z_); }
/* plus operator overload */
Point3D Point3D::operator+(const Point3D& rhs) const { return Point3D(x_ + rhs.x_, y_ + rhs.y_, z_ + rhs.z_); }
/* division operator overload */
Point3D Point3D::operator/(const float rhs) const { return Point3D(this->x_ / rhs, this->y_ / rhs, this->z_ / rhs); }
/* multiplication operator overload */
Point3D Point3D::operator*(const float rhs) const { return Point3D(this->x_ * rhs, this->y_ * rhs, this->z_ * rhs); }
/* equality operator overload */
bool Point3D::operator==(const Point3D& rhs) const { return x_ == rhs.x_ && y_ == rhs.y_ && z_ == rhs.z_; }
/* inequality operator overload */
bool Point3D::operator!=(const Point3D& rhs) const { return x_ != rhs.x_ || y_ != rhs.y_ || z_ != rhs.z_; }
/* returns the cross product of the current point with the given one */
Point3D Point3D::Cross(const Point3D& point) const {
    return Point3D(y_ * point.z_ - z_ * point.y_, z_ * point.x_ - x_ * point.z_, x_ * point.y_ - y_ * point.x_);
}
...
void Point3D::SetCoords(std::tuple<float, float, float> tuple) { std::tie(x_, y_, z_) = tuple; }
...
void Point3D::SetY(float y) { y_ = y; }
void Point3D::SetZ(float z) { z_ = z; }

/* function to compute the euclidean distance between two points */
float Point3D::ComputeDistance(const mapobjects::Point3D& point) const {
    return std::sqrt(std::pow(x_ - point.x_, 2) + std::pow(y_ - point.y_, 2) + std::pow(z_ - point.z_, 2));
}
/* function to compute the euclidean distance between two points */
float Point3D::ComputeDistance(float x, float y, float z) const {
    return std::sqrt(std::pow(x_ - x, 2) + std::pow(y_ - y, 2) + std::pow(z_ - z, 2));
}
/* sets the lane uuid */
float Point3D::GetNorm() const { return std::sqrt(x_ * x_ + y_ * y_ + z_ * z_); }
float Point3D::GetSquaredNorm() const { return x_ * x_ + y_ * y_ + z_ * z_; }
...
mapobjects::Pose Pivot::GetPose() const { return pose_; }
void Pivot::SetPose(const Pose& pose) { pose_ = pose; }
// ____________________________________________________________________________
// getters for LaneMarkingContainer
std::vector<mapobjects::Uuid> LaneMarkingContainer::GetLeftLaneMarkings() const { return left_; }
std::vector<mapobjects::Uuid> LaneMarkingContainer::GetRightLaneMarkings() const { return right_; }

// ____________________________________________________________________________
// getters for LaneGroup
std::vector<mapobjects::Uuid> LaneGroup::GetLeftLanes() const { return lanes_left_; }
std::vector<mapobjects::Uuid> LaneGroup::GetRightLanes() const { return lanes_right_; }
/* detects whether the current lanegroup is a junction */
void LaneGroup::SetAsJunction(bool is_junction) { is_junction_ = is_junction; }
/* is the lanegroup a junction */
bool LaneGroup::IsJunction() const { return is_junction_; }
// ____________________________________________________________________________
// getters for Lane
std::vector<const Roadsign*> Lane::GetRoadSigns() const { return road_signs_; }

bool Lane::HasRoadSign(std::string sign_name) const {
    auto result = std::find_if(road_signs_.begin(), road_signs_.end(),
                               [sign_name](const Roadsign* rs) { return rs->GetSignType() == sign_name; });
    return result != road_signs_.end();
}
const Stopline* Lane::GetStopLine() const { return stopline_; }
void Lane::AddRoadSign(const Roadsign* sign) { road_signs_.emplace_back(sign); }
...
mapobjects::Point3D Lane::GetHandlePoint() const { return handle_point_; }
mapobjects::Point3D& Lane::GetHandlePointByReference() { return handle_point_; }
...
mapobjects::LaneMarkingContainer Lane::GetLaneMarkingContainer() const { return lane_markings_; }
mapobjects::LaneDirection Lane::GetLaneDirection() const { return dir_; }
mapobjects::LaneType Lane::GetLaneType() const { return type_; }
std::vector<mapobjects::Uuid> Lane::GetOutConnections() const { return connections_out_; }
std::vector<mapobjects::Uuid> Lane::GetInConnections() const { return connections_in_; }
std::vector<mapobjects::Point3D> Lane::GetPoints() const { return points_; }
bool Lane::GetVisibility() const { return visibility_; }
int Lane::GetHeight() const { return height_; }
int Lane::GetJunctionID() const { return junction_id_; }
float Lane::GetLength() const { return length_; }
/* returns a reference to the vector lane points. use: making a dense node graph  */
std::vector<Point3D>& Lane::GetPointsByReference() { return points_; }
/* sets the parent Uuid, post-hoc */
void Lane::SetParentUuid(const Uuid& parent_uuid) { parent_uuid_ = parent_uuid; }
/* retunrs the parent lanegroup' uuid */
Uuid Lane::GetParentUuid() const { return parent_uuid_; }
...
[[deprecated("deprecated function")]] std::pair<Point3D, int> Lane::GetClosestLanePoint(
    const mapobjects::Point3D& point, bool reverse, int start_index) const {
    float last_dist = 1e6;
    float min_dist = 1e6;
    int index = -1;
    if (reverse) {
        for (int i = (start_index == -1) ? points_.size() - 1 : start_index; i >= 0; --i) {
            ...
            // hit global minimum
            if (current_dist > last_dist) break;
...
const Lane* Lane::GetConnection(Connection connection) const {
    ...
    if (it != connection_hash_->end()) return it->second;
    return nullptr;
}
...
Lane::Connection Lane::GetConnectionType(const Lane& lane) const {
    for (auto& it : *connection_hash_) {
        if (it.second == &lane) return it.first;
    }
...
void Lane::PrintLane() const {
    ...
    for (auto& it : *connection_hash_) {
        std::cout << "\t" << GetConnectionString(it.first) << " to " << it.second->GetUuid().GetUuidValue()
                  << std::endl;
    }
}
bool Lane::IsJunctionLane() const { return junction_id_ != -1; }
/* returns whether this is the dummy lane that represents offlane */
bool Lane::IsOffroad() const { return id_.GetUuidValue() == "?"; }
// ____________________________________________________________________________
// getters for LaneMarking
mapobjects::LaneMarkingType LaneMarking::GetLaneMarkingType() const { return type_; }
bool LaneMarking::GetVisibility() const { return visibility_; }
std::vector<mapobjects::Point3D> LaneMarking::GetPoints() const { return points_; }
std::vector<Point3D>& LaneMarking::GetPointsByReference() { return points_; }
...
float Pose::x() const { return x_; }
float Pose::y() const { return y_; }
float Pose::z() const { return z_; }
float Pose::t() const { return fmod(t_, 2 * M_PI); }
mapobjects::Point3D Pose::GetPoint3D() const { return mapobjects::Point3D(this->x_, this->y_, this->z_); }
...
mapobjects::LaneObjectType LaneObject::GetLaneObjectType() const { return type_; }
mapobjects::Uuid LaneObject::GetLaneGroupId() const { return lg_id_; }
std::vector<mapobjects::Uuid> LaneObject::GetLaneIds() const { return lane_ids_; }
float LaneObject::GetOffset() const { return offset_; }

// ____________________________________________________________________________
// getters for LaneObject::Parking
int Parking::GetNumberOfParkingLots() const { return n_parking_lots_; }
float Parking::GetWidth() const { return parking_width_; }
float Parking::GetHeight() const { return parking_height_; }
float Parking::GetLineWidth() const { return linewidth_; }
std::vector<std::vector<mapobjects::Point3D>> Parking::GetOuterPoints() const { return outer_points_; }
// ____________________________________________________________________________
// getters for LaneObject::Roadsign

std::string Roadsign::GetSignType() const { return sign_type_; }
Point3D Roadsign::GetPosition() const { return position_; }
float Roadsign::GetRotation() const { return rotation_; }
/* sets the roadsign position (used for subtracting the pivot coordinates) */
void Roadsign::SetPosition(float x, float y, float z) { position_.SetCoords(x, y, z); }
// ____________________________________________________________________________
// getter for LaneObject::StopLine
Point3D Stopline::GetPosition() const { return position; }
...
void Junction::AddLane(const Lane& lane) {
    if (lane.GetJunctionID() == -1) {
        std::cout << "ERROR: lane " << lane.GetUuid().GetUuidValue() << " is not a junction lane but used as one"
                  << std::endl;
        return;
    } else if (lane.GetJunctionID() != id_) {
        std::cout << "ERROR: lane " << lane.GetUuid().GetUuidValue() << " is not a part of junction #" << id_
                  << std::endl;
        return;
    }
    // adding lanes
    ...
    if (!IsBoundaryPoint(lane_points.front())) boundary_points_.emplace_back(lane_points.front());
    if (!IsBoundaryPoint(lane_points.back())) boundary_points_.emplace_back(lane_points.back());
    // adding lanegroup
    ...
    if (new_lanegroup) lanegroups_.emplace_back(lane.GetParentUuid());
}
...
inline bool Junction::IsBoundaryPoint(const Point3D& point) const {
    for (auto& boundary_point : boundary_points_) {
        if (point == boundary_point) return true;
    }
    return false;
}
/* returns the (sorted) boundary points */
std::vector<Point3D> Junction::GetBoundaryPoints() const { return boundary_points_; }
/* returns the handle point for the junction */
Point3D Junction::GetHandlePoint() const { return handle_point_; }
std::vector<Uuid> Junction::GetLaneGroups() const { return lanegroups_; }
...
void Junction::PostProcess() {
    ...
    for (size_t i = 0; i < boundary_points_.size(); ++i) handle_point_ = handle_point_ + boundary_points_[i];
    ...
    type_ = (incoming_lanes_.size() == 3) ? Type::T_INTESERSECTION
                                          : (incoming_lanes_.size() == 4) ? Type::CROSS_INTESERSECTION : Type::NONE;
...
std::vector<Uuid> Junction::GetLanes() const { return lanes_; }
/* returns a list of lanes that lead to the junction */
std::vector<Uuid> Junction::GetIncomingLanes() const { return incoming_lanes_; }
/* returns a list of lanes that exit the junction */
std::vector<Uuid> Junction::GetOutgoingLanes() const { return outgoing_lanes_; }
int Junction::GetID() const { return id_; }
Junction::Type Junction::GetType() { return type_; }

// ____________________________________________________________________________
// LanePoint3D class
std::string LanePoint3D::GetLaneUuid() { return lane_uuid_; }
unsigned int LanePoint3D::GetIndex() { return index_; }
float LanePoint3D::GetHeading() { return heading_; }
float LanePoint3D::GetLaneOffset() { return lane_offset_; }
Point3D LanePoint3D::AsPoint3D() { return Point3D(x_, y_, z_); }
