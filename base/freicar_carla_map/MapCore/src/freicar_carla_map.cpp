...
MapStatus Map::status() const { return status_; }

/* returns the pivot point*/
mapobjects::Pivot Map::pivot() const { return pivot_point_; }
...
std::tuple<int, int, int> Map::GetMapStats() const {
    return std::make_tuple(lane_storage_.size(), lanegroup_storage_.size(), lanemarking_storage_.size());
}
...
map_thrift::MapMessage Map::AsThriftMessagae() const {
    ...
    map_thrift::MapPart m_part = create::MapPartFromMapObjects(
        convert::ToThriftLaneMarkingBatch(lanemarking_storage_), convert::ToThriftLaneBatch(lane_storage_),
        convert::ToThriftLaneGroupBatch(lanegroup_storage_), laneobject_list);
...
void Map::AddLaneGroup(const mapobjects::LaneGroup& lane_group) {
    ...
    if (index == -1) {
        try {
            ...
            // map uuid to index
            lanegroup_uuids_.insert(
                {(lane_group.GetUuid()).GetUuidValue(), (static_cast<int>(lanegroup_storage_.size()) - 1)});
        } catch (const std::invalid_argument& ia) {
            std::cout << "[ERROR] mapobject is of invalid type!" << std::endl;
        }
    } else if (index > -1) {
...
std::vector<mapobjects::Lane> Map::getLanes() { return lane_storage_; }
std::vector<mapobjects::Roadsign> Map::getSigns() { return roadsign_storage_; }
...
void Map::AddToStoplinesLookup(mapobjects::Stopline stopline) {
    ...
    for (int i = 0; i < n_lanes_to_process; ++i) {
        ...
        if (it == lane_stoplines_lookup_.end()) {
            lane_stoplines_lookup_.insert(
                {stopline_lanes[i].GetUuidValue(), std::vector<mapobjects::Uuid>(1, stopline.GetUuid())});
        } else {
...
void Map::AddToCrosswalksLookup(mapobjects::Crosswalk crosswalk) {
    ...
    for (int i = 0; i < n_lanes_to_process; ++i) {
        ...
        // if lane not in lookup
        if (it == lane_stoplines_lookup_.end())
            lane_crosswalks_lookup_.insert(
                {crosswalk_lanes[i].GetUuidValue(), std::vector<mapobjects::Uuid>(1, crosswalk.GetUuid())});
        else
...
void Map::AddToParkingLookup(mapobjects::Parking parking) {
    ...
    for (int i = 0; i < n_lanes_to_process; ++i) {
        ...
        if (it == lane_parking_lookup_.end())
            lane_parking_lookup_.insert(
                {parking_lanes[i].GetUuidValue(), std::vector<mapobjects::Uuid>(1, parking.GetUuid())});
        else
...
void Map::AddToRoadsignLookup(mapobjects::Roadsign roadsign) {
    ...
    for (int i = 0; i < n_lanes_to_process; ++i) {
        ...
        if (it == lane_stoplines_lookup_.end())
            lane_stoplines_lookup_.insert(
                {stopline_lanes[i].GetUuidValue(), std::vector<mapobjects::Uuid>(1, roadsign.GetUuid())});
        else
...
std::vector<mapobjects::Uuid> Map::GetPureOutgoingConnections(const mapobjects::Lane& lane) {
    ...
    for (auto& out_connection : lane.GetOutConnections()) {
        ...
        if (out_lane->GetParentUuid() != lane_group->GetUuid()) pure_out.emplace_back(out_lane->GetUuid());
    }
    return pure_out;
}
...
std::vector<mapobjects::Uuid> Map::GetPureIncomingConnections(const mapobjects::Lane& lane) {
    ...
    for (auto& in_connection : lane.GetInConnections()) {
        ...
        if (in_lane->GetParentUuid() != lane_group->GetUuid()) pure_in.emplace_back(in_lane->GetUuid());
    }
    return pure_in;
}
/* returns the closest lane point and the squared distance to a given position using kd-tree search */
std::vector<std::pair<mapobjects::LanePoint3D, float>> Map::FindClosestLanePoints(float local_x, float local_y,
                                                                                  float local_z,
                                                                                  unsigned int count) const {
    float kd_point[3] = {local_x, local_y, local_z};
    std::vector<unsigned int> ret_index(count);
    ...
    for (size_t i = 0; i < count; ++i) ret_nodes.emplace_back(kd_points_[ret_index[i]], out_dist_sqr[i]);
    return ret_nodes;
}
/* returns the closest lane point and the squared distance to a given position using kd-tree search, sorted by heading
 * (-PI, PI)*/
std::vector<std::pair<mapobjects::LanePoint3D, float>> Map::FindClosestLanePointsWithHeading(
    float local_x, float local_y, float local_z, unsigned int count, float heading) const {
    float kd_point[3] = {local_x, local_y, local_z};
    std::vector<unsigned int> ret_index(count);
    std::vector<float> out_dist_sqr(count);
    count = kd_tree_->knnSearch(&kd_point[0], count, &ret_index[0], &out_dist_sqr[0]);
    ...
    std::sort(permutation.begin(), permutation.end(), [ret_index, heading, this](unsigned int a, unsigned int b) {
        return std::abs(kd_points_[ret_index[a]].heading_ - heading) <
               std::abs(kd_points_[ret_index[b]].heading_ - heading);
    });
...
mapobjects::LanePoint3D Map::GetRandomLanePoint() const { return kd_points_.at(rand() % kd_points_.size()); }
...
/* return random lane */
std::vector<mapobjects::Junction> Map::GetJunctions() { return junction_storage_; }
...
void Map::PostProcess(float density_cm) {
    ...
    Lane dummy(id, container, LaneDirection::LEFT, empty, empty, LaneType::PEDESTRIAN_LANE, empty_p, 0, Point3D(),
               false, 0, -1);
    AddLane(dummy);
}
...
void Map::InitializeJunctions() {
    for (auto& lane : lane_storage_) {
        ...
        if (lane.IsJunctionLane()) {
            ...
            for (int i = junction_storage_.size(); i - 1 < junc_idx; ++i) junction_storage_.emplace_back(i);
...
void Map::AdjustCoordinates() {
    ...
    for (auto& roadsign : roadsign_storage_) {
        ...
        roadsign.SetPosition(pos.x() - pivot_point_.GetPose().x(), pos.y() - pivot_point_.GetPose().y(),
...
void Map::FillLaneHashMaps() {
    for (auto& current_lane : lane_storage_) {
        ...
        if (interconnections.size() == 1) {
            ...
            if (!current_lane.IsJunctionLane()) next_lane->RegisterConnection(mapobjects::Lane::BACK, &current_lane);
        } else {
            // junctions
            ...
            assert(int(current_lanepoints.size()) - 2 > 0);
            ...
            for (auto& junction_uuid : interconnections) {
                ...
                if (!junction_lane->IsJunctionLane()) continue;

                assert(!junction_lane->GetOutConnections().empty());
                ...
                assert(!dest_lane->GetPointsByReference().empty());
                ...
                if (relative_direction == 0) {
                    ...
                    if (angle_cos >= config::kJunctionStraightMargin)
                        current_lane.RegisterConnection(mapobjects::Lane::JUNCTION_STRAIGHT, junction_lane);
                    else
...
void Map::CreateKDIndex() {
    ...
    kd_tree_ = new nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<float, Map>, Map, 3>(
        3, *this, nanoflann::KDTreeSingleIndexAdaptorParams(10));
    kd_tree_->buildIndex();
...
void Map::SendAsRVIZMessage(float point_size, float lanemarking_size, std::shared_ptr<ros::NodeHandle> n) {
    ...
    if (!marker_pub_)
        marker_pub_ =
            std::make_unique<ros::Publisher>(n->advertise<visualization_msgs::MarkerArray>("map_vis_marker", 10, true));
    ...
    for (auto& lanemarking : lanemarking_storage_) {
        ...
        if (!lanemarking.GetVisibility()) continue;
    ...
    for (auto& lane : lane_storage_) {
        ...
        geometry_msgs::Point pt;
        auto point3d =
            stopline_ptr->position + stopline_ptr->v_forward * 0.03 + stopline_ptr->v_right * lane.GetWidth() / 2;
        pt.x = point3d.x();
        pt.y = point3d.y();
        pt.z = point3d.z();
...
std::tuple<int, int, int> Map::GetNeighboringLanes(const mapobjects::Lane& lane,
                                                   const mapobjects::LaneGroup& lane_group) {
        ...
        if (adj_lane->GetParentUuid() == lane_group.GetUuid()) {
            if (!lane_group.IsJunction()) {
                ...
                if (adj_lane->GetLaneDirection() == lane.GetLaneDirection()) {
                    ...
                    if (relative_direction == 1) adjacent_right_idx = i;
                    if (relative_direction == -1) adjacent_left_idx = i;
                }
...
int Map::GetUpcomingJunctionID(const std::string& lane_uuid_value) const {
    ...
    if (query == incoming_lane_junction_lookup_.end()) return -1;
    return query->second;
}
/* returns the ID of the junction that the current lane leads to, 0 otherwise */
