...
enum LaneDirection { RIGHT = 1, LEFT = 2 };
   ...
   public:
    ...
    LaneInfo(bool overtake_in, bool stoplines_in, bool crosswalks_in, bool parking_in)
        : no_overtaking(overtake_in),
          has_stoplines(stoplines_in),
          has_crosswalks(crosswalks_in),
          has_parking(parking_in) {}
};
...
class LaneMarkingContainer {
   public:
    LaneMarkingContainer(std::vector<Uuid> left_lm, std::vector<Uuid> right_lm) : left_(left_lm), right_(right_lm) {}
...
class MapObject {
   ...
   public:
    ...
    /* make  ~tor pure virtual with virtual ...() = 0 to prevent instantiating.
     * Must instantiate child objects */
    virtual ~MapObject() = 0;
};
...
class Pose {
   ...
   public:
    ...
    Pose(float x_in, float y_in, float z_in) : x_(x_in), y_(y_in), z_(z_in), t_(0) {}
    Pose(float x_in, float y_in, float z_in, float t_in) : x_(x_in), y_(y_in), z_(z_in), t_(t_in) {}
...
class Stopline : public LaneObject {
   ...
   private:
    // set in Lane::SetStopline, used for visualizations in Map
...
class Parking : public LaneObject {
   public:
    Parking(Uuid id_in, Uuid lg_id_in, std::vector<Uuid> lane_ids_in, float offset_in, int n_parking_lots_in,
            float parking_height_in, float parking_width_in, float linewidth_in,
            std::vector<std::vector<Point3D>> outer_points_in)
        : LaneObject(id_in, LaneObjectType::LO_PARKING, lg_id_in, lane_ids_in, offset_in),
          n_parking_lots_(n_parking_lots_in),
          parking_height_(parking_height_in),
          parking_width_(parking_width_in),
          linewidth_(linewidth_in),
          outer_points_(outer_points_in) {}
    ~Parking();
...
class Roadsign : public LaneObject {
   public:
    Roadsign(Uuid id_in, Uuid lg_id_in, std::vector<Uuid> lane_ids_in, float offset_in, std::string sign_type_in,
             mapobjects::Point3D position_in, float rotation_in)
        : LaneObject(id_in, LaneObjectType::LO_SIGN, lg_id_in, lane_ids_in, offset_in),
          sign_type_(sign_type_in),
...
class Lane : public MapObject {
   public:
    ...
    Lane(Uuid id_in, LaneMarkingContainer lm, LaneDirection d, std::vector<Uuid> c_in, std::vector<Uuid> c_out,
         LaneType t, std::vector<Point3D> p, float w, Point3D hp, bool v, int h, int j_id)
        : MapObject(id_in),
          lane_markings_(lm),
          dir_(d),
          connections_out_(c_out),
          connections_in_(c_in),
          type_(t),
          points_(p),
          width_(w),
          handle_point_(hp),
        ...
        hash_lambda_ = [](const Connection& type) { return std::hash<int>()(static_cast<int>(type)); };
        ...
        if (points_.size() > 1) {
            for (size_t i = 0; i < points_.size() - 1; ++i) length_ += points_[i].ComputeDistance(points_[i + 1]);
        }
    }
    ~Lane();
   ...
   public:
    ...
    std::pair<Point3D, int> GetClosestLanePoint(const mapobjects::Point3D& point, bool reverse = false,
                                                int start_index = -1) const;
...
class Junction {
   public:
    enum Type { T_INTESERSECTION, CROSS_INTESERSECTION, ROUNDABOUT, NONE };
