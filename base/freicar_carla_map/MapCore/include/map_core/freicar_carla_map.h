...
enum MapStatus { UNINITIALIZED = 1, LOADED_FROM_FILE = 3, UPDATED_FROM_REMOTE = 4 };
...
class Map {
   public:
    ...
    std::vector<std::pair<mapobjects::LanePoint3D, float>> FindClosestLanePoints(float local_x, float local_y,
                                                                                 float local_z,
                                                                                 unsigned int count) const;
    std::vector<std::pair<mapobjects::LanePoint3D, float>> FindClosestLanePointsWithHeading(
    ...
    std::tuple<int, int, int> GetNeighboringLanes(const mapobjects::Lane& lane,
   ...
   private:
    ...
    // kd-tree
    nanoflann::KDTreeSingleIndexAdaptor<nanoflann::L2_Simple_Adaptor<float, Map>, Map, 3>* kd_tree_;
    // utility functions
