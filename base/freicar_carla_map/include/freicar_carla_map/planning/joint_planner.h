...
class LaneFollowerStrategy : public PlanningStrategy {
   public:
    ...
    LaneFollowerStrategy(const std::string& lane_uuid, float req_loffset, enums::PlannerCommand command, float distance,
                         unsigned int step_count)
        : lane_uuid_(lane_uuid),
          req_loffset_(req_loffset),
          command_(command),
          distance_(distance),
          step_count_(step_count) {}
    LaneFollowerStrategy(const mapobjects::Point3D& current_position, enums::PlannerCommand command, float distance,
                         unsigned int step_count)
        : start_position_(current_position), command_(command), distance_(distance), step_count_(step_count) {}
    ...
    planning::Plan Extend() override {
        planning::Plan extension = lane_follower::GetPlan(last_plan_step_.lane_uuid, last_plan_step_.lane_offset,
                                                          command_, distance_, step_count_);
        last_plan_step_ = extension[-1];
        return extension;
    }
    enums::PlannerCommand GetCommand() override { return command_; }
    ...
    bool IsExtendable() override { return true; }
...
class LaneStarStrategy : public PlanningStrategy {
   public:
    ...
    LaneStarStrategy(mapobjects::Point3D& start, float start_heading, mapobjects::Point3D& goal, float goal_heading,
                     float p2p_distance, unsigned int maximum_steps)
        : start_(start),
          goal_(goal),
          start_heading_(start_heading),
          goal_heading_(goal_heading),
          p2p_distance_(p2p_distance),
          planner_(maximum_steps) {}
    planning::Plan GetPlan() override {
    ...
    enums::PlannerCommand GetCommand() override { return enums::PlannerCommand::POINT; }
    std::string GetStrategyString() override { return "lane star"; }
    bool IsExtendable() override { return false; }
...
class DirectPathStrategy : public PlanningStrategy {
   public:
    ...
    planning::Plan GetPlan() override {
        ...
        new_plan.steps.emplace_back(start_, mapobjects::Lane::Connection::STRAIGHT, "?", 0.0f, 0.0f);
        new_plan.steps.emplace_back((start_ + goal_) / 2, mapobjects::Lane::Connection::STRAIGHT, "?", 0.0f, 0.0f);
        new_plan.steps.emplace_back(goal_, mapobjects::Lane::Connection::STRAIGHT, "?", 0.0f, 0.0f);
        return new_plan;
    }
    planning::Plan Replan(float x, float y, float z, float heading = 0) override {
        start_.SetCoords(x, y, z);
    ...
    enums::PlannerCommand GetCommand() override { return enums::PlannerCommand::DIRECT; }
    std::string GetStrategyString() override { return "direct path"; }
    bool IsExtendable() override { return false; }
...
class EmptyStrategy : public PlanningStrategy {
   public:
    ...
    enums::PlannerCommand GetCommand() override { return enums::PlannerCommand::EMPTY; }
    std::string GetStrategyString() override { return "empty"; }
    bool IsExtendable() override { return false; }
};
}  // namespace strategy
...
class JointPlanner {
   public:
    JointPlanner() { strategy_ = nullptr; }
    ...
    planning::Plan Replan(float x, float y, float z, float heading) { return strategy_->Replan(x, y, z, heading); }
    planning::Plan Extend() { return strategy_->Extend(); }
    enums::PlannerCommand GetCommand() { return strategy_->GetCommand(); }
    std::string GetStrategyString() { return strategy_->GetStrategyString(); }
    bool IsExtendable() { return strategy_->IsExtendable(); }
