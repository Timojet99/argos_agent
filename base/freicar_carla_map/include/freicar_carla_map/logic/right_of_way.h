...
#include "freicar_carla_map/FreiCarAgentLocalization.h"
#include "freicar_carla_map/logic/map_info.h"
namespace freicar {
namespace logic {
struct JunctionAgent : public AgentMapInfo {
    ...
    enum Intent : unsigned char { GOING_STRAIGHT = 0, GOING_LEFT = 1, GOING_RIGHT = 2, NONE = 7 };
    JunctionAgent() {}
    JunctionAgent(const freicar_carla_map::FreiCarAgentLocalization& loc_msg)
    ...
    JunctionAgent(const std::string& a_name, const std::string& l_uuid, float l_offset,
                  const geometry_msgs::Vector3& vel, const geometry_msgs::TransformStamped& pose,
                  Intent intent = Intent::NONE)
        : AgentMapInfo(a_name, l_uuid, l_offset, vel), intent(intent) {
        current_pose = pose;
    }
    JunctionAgent(const AgentMapInfo& a_info, Intent intent = Intent::NONE)
        : AgentMapInfo(a_info.name, a_info.lane_uuid, a_info.lane_offset, a_info.GetVelocityVector()), intent(intent) {
...
std::tuple<bool, bool, std::string> GetRightOfWay(const JunctionAgent& agent,
                                                  ...
                                                  bool consider_velocities = false, bool ignore_intent = false);
