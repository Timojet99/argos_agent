...
class ThriftMapProxy {
   public:
    ThriftMapProxy(const std::string& remote_address = "192.168.168.127", int remote_port = 9091,
                   int local_server_port = 9090);
    ~ThriftMapProxy();
    ...
    class ThriftClient {
       public:
        ThriftClient(const std::string& address = "192.168.168.127", int port = 9091)
            : remote_thrift_server_address_(address), remote_thrift_server_port_(port), thrift_client_object_(nullptr) {
